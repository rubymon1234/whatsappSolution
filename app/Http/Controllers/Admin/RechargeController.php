    ...
    */
   	public function getRequestView(Request $request)
    {
        ...
        $plans = DB::table('plan_requests')
            ...
            ->select('plans.plan_name','plans.id as plan_id','plan_requests.id as id','plans.daily_count','plans.plan_validity','plan_requests.credit','plan_requests.created_at','users.name as user_name','plan_requests.reseller_id','plans.scrub_count','plans.bot_instance_count','plans.plan_subscription')
            ->where('plan_requests.is_status','=','2') //pending for approval
            ->orderBy('plan_requests.updated_at','DESC') 
            ->paginate(10);
