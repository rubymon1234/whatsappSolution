...
class InboundMessagesController extends Controller
{
     ...
     * @author Ruban
    */
    public function getInboundView(Request $request){

        $instanceList = Instance::where('user_id',Auth::user()->id)
        ...
        if($number !=''){
            $condtion.=" AND wc_inbound_messages.`number`='".$number. "'";
        }
        if($instance_id !='') {
            $instanseDetail = Instance::find($instance_id);
            $instance_id = $instanseDetail->id;
            $condtion.=" AND wc_inbound_messages.`instance_token`='".$instanseDetail->token. "'";
        }
        $inboundMessages =\DB::table('inbound_messages')
                    ->join('instances', 'instances.token', '=', 'inbound_messages.instance_token')
                    ->whereRaw("wc_inbound_messages.`user_id` = '".$user_id. "' $condtion")
                    ->select('instances.*','inbound_messages.*')
     ...
     * @author Ruban
    */
    public function postInBoundMessagePush(Request $request){
        ...
        if($request->ajax()) {
            ...
            if ($validator->fails()) {
                ...
                if($validResponse['status'] ==true && $extensionValidation['status'] ==true){
                    ...
                    $num_count =0;
                    $csv_name ='';
                    if(isset($request->bulkData)){
                        //$csvDetail = $this->createCsv($request->mobile);
                        $multipleInstanceMobile = array();
                        $num_countArray = array();
                        foreach (json_decode($request->bulkData) as $k => $bulk) {
                            $key = $k;
                            if(!empty($multipleInstanceMobile)){
                                foreach ($multipleInstanceMobile as $index => $multi) {
                                    if(isset($multi[$bulk->instance_token])){
                                        $key = $index;
                                        break;
                                    }else{
                                        $key = $k;
                    ...
                    if(isset($campaignFetch[0]['total'])){ $total = $campaignFetch[0]['total']; }else{ $total = 0; }
                        ...
                        if($plan_validity >= $today_date){
                            foreach ($multipleInstanceMobile as $keyy => $rslt) {
                            //current instance
                            $instance_id = array_keys($rslt)[0];
                            $csvDetail = $this->createCsv($rslt[$instance_id]);
                            $csv_name = $csvDetail['csv_name'];
                            $num_count = count($rslt[$instance_id]);
                            $getInstance = Instance::where('token',$instance_id)->where('is_status',1)->first();
                            if($getInstance){
                                //Campaign Creation
                                $campaignInsert = new Campaign();
                                $campaignInsert->campaign_name = $campaign.'_'.$instance_id;
                                ...
                                $campaignInsert->count = count($rslt[$instance_id]);
                                if($num_count <=10){
